...
class PretrainTokenizerDataset(Dataset):
    """
    用于预训练tokenizer的数据集类，支持从多个CSV文件加载数据
    与微调数据集的主要区别：
    1. 支持多文件批量加载
    2. 数据量更大，需要更高效的采样策略
    3. 跨股票/市场的随机采样
...
def setup_logging(exp_name: str, log_dir: str, rank: int = 0) -> logging.Logger:
    ...
    logger = logging.getLogger(f"tokenizer_pretrain_rank_{rank}")
    logger.setLevel(logging.INFO)
    
    if logger.handlers:
        return logger
    
    log_file = os.path.join(log_dir, f"tokenizer_pretrain_rank_{rank}.log")
    ...
    logger.info(f"=== Tokenizer Pretraining Started ===")
...
def create_dataloaders(config):
    ...
    train_dataset = PretrainTokenizerDataset(
    ...
    val_dataset = PretrainTokenizerDataset(
        data_dir=config.data_path,
        data_type='val',
        lookback_window=config.lookback_window,
        predict_window=config.predict_window,
        ...
        val_ratio=config.val_ratio,
        test_ratio=config.test_ratio,
        max_files=getattr(config, 'max_files', 10),  # 验证集只使用前10个文件
...
def train_tokenizer(tokenizer, device, config, save_dir, logger):
    """训练tokenizer"""
    ...
    if use_ddp:
        ...
        tokenizer = DDP(tokenizer, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False)
    ...
    for epoch in range(config.tokenizer_epochs):
        ...
        for batch_idx, batch_x in enumerate(train_loader):
            ...
            # 前向传播
            (z_pre, z), bsq_loss, quantized, z_indices = (tokenizer.module if use_ddp else tokenizer)(batch_x)
            
            # 计算重构损失
            recon_loss = nn.MSELoss()(z, batch_x)
            total_loss = recon_loss + bsq_loss  # 总损失包括重构损失和量化损失
            
            optimizer.zero_grad()
            total_loss.backward()
            torch.nn.utils.clip_grad_norm_((tokenizer.module if use_ddp else tokenizer).parameters(), max_norm=3.0)
            optimizer.step()
            scheduler.step()
            ...
            if (batch_idx_global + 1) % config.log_interval == 0:
                ...
                log_msg = (f"[Epoch {epoch+1}/{config.tokenizer_epochs}, Step {batch_idx+1}/{len(train_loader)}] "
                          f"LR: {lr:.6f}, Total Loss: {total_loss.item():.4f}, "
                          f"Recon Loss: {recon_loss.item():.4f}, Bsq Loss: {bsq_loss.item():.4f}")
                logger.info(log_msg)
                if rank == 0:
                    print(log_msg)
        ...
        with torch.no_grad():
            for batch_x in val_loader:
                ...
                (z_pre, z), bsq_loss, quantized, z_indices = (tokenizer.module if use_ddp else tokenizer)(batch_x)
                recon_loss = nn.MSELoss()(z, batch_x)
                total_loss = recon_loss + bsq_loss
        ...
        if avg_val_loss < best_val_loss:
            ...
            if rank == 0:
                tokenizer_save_path = os.path.join(save_dir, "best_tokenizer")
                os.makedirs(tokenizer_save_path, exist_ok=True)
                (tokenizer.module if use_ddp else tokenizer).save_pretrained(tokenizer_save_path)
                save_msg = f"Best tokenizer saved to: {tokenizer_save_path} (validation loss: {best_val_loss:.4f})"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Kronos Tokenizer Pretraining')
    ...
    log_dir = os.path.join(config.tokenizer_save_path, "logs")
    ...
    # 预训练模式下，不加载预训练权重，随机初始化tokenizer
    logger.info("Random initialization for tokenizer pretraining (not loading pretrained weights)...")
    print("Random initialization for tokenizer pretraining (not loading pretrained weights)...")
    ...
    logger.info("=== Tokenizer Pretraining Configuration ===")
    ...
    logger.info("Starting tokenizer pretraining...")
    print("Starting tokenizer pretraining...")
    best_val_loss = train_tokenizer(tokenizer, device, config, config.tokenizer_save_path, logger)
    
    final_msg = f"Tokenizer pretraining completed! Best validation loss: {best_val_loss:.4f}\nTokenizer saved to: {config.tokenizer_save_path}"
